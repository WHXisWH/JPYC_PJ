...
export class LedgerService {
  createPurchaseTx(input: { planId: string; venueId: string } & { currency: 'JPYC'; amountMinor: number }) {
    MoneySchema.parse({ currency: input.currency, amountMinor: input.amountMinor });
    const seed = `${input.planId}:${input.venueId}:${input.currency}:${input.amountMinor}`;
